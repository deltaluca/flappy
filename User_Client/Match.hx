...
class Match {
	...
	static function extract(e:Expr, b:Expr, ?cnt:Int=0) {
		function gen() {
			var name = "__matchnull_"+(cnt++);
			var guard = mk(EBinop(OpEq, mk(EConst(CIdent(name))), mk(EConst(CIdent("null")))));
			if(b!=null) guard = mk(EBinop(OpBoolAnd, b, guard));
			return {expr:mk(EConst(CIdent(name))), guard:guard, cnt:cnt};
		}
		switch(e.expr) {
			case EConst(c):
				switch(c) {
			...
			case ECall(a,xs):
				...
				return { expr:mk(ECall(a,ys)), guard:b, cnt:cnt};
			default:
		}
		return {expr:e,guard:b,cnt:cnt};
	}
					...
					switch(op) {
						...
						case OpAssign:
							...
							if(cont) {
								...
								cases.push({value:vg.expr,expr:block,guard:vg.guard});
							}
							else def = block;
					}
			}
		...
		switch(cs.expr) {
			...
			case ECall(en,args):
				...
				return mk(ESwitch(match,
					[{ expr: recurse,
					   values: [mk(ECall(mk(EConst(CIdent(name))),switchvars))] }],
					mk(EBlock([]))
				));
		}
	}
	...
	static function domatch(e:Array<Expr>):Expr {
		...
		var states = [];
		states.push(mk(EVars([{ name: matchon, type: null, expr: gdata.on }])));
		...
		if(gdata.def!=null)
			states.push(mk(EReturn(gdata.def)));
		else
			states.push(mk(EThrow(mk(EConst(CString("Non-exhaustive patterns in match"))))));
   ...
   static function pretty(s: String) {
	...
	@:macro
	public static function match(e:Array<haxe.macro.Expr>) {
		var ret = domatch(e);
//		trace(pretty(Std.string(ret)));
		return ret;
	}
}
