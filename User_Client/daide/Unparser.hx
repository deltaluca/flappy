...
class Unparser{
    ...
    static function ununit_with_location_and_mrt(x:UnitWithLocAndMRT):Array<Token>{
        ...
        else ununit_with_location(x.unitloc).concat([MRT,tLeftParen]).concat(unmdf_province_list(x.locs)).concat([tRightParen]);
    }
    ...
    static function unsc_ownership_list(x:ScOwnershipList):Array<Token>{
        return[if(x.power==null)UNO else tPower(x.power)].concat(unprovince_list(x.locs));
    }
