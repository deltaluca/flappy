...
mapProvinceNode pn@(ProvNode prov) = (prov, pn)
...
pSupplyCOs = do
  scos <- many . paren $ do
    ...
    return (pow, centres)
  return . (foldl (\m (k, v) -> Map.insert k v m) Map.empty) $ scos
