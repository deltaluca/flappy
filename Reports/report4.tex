...
algorithm \cite{Shapiro02} coupled with a best-first search strategy is used.
In addition a DAIDE-conforming \cite{DaideWeb} subsystem comprising
GUI client and server was implemented. Taking both together enabled us to 
conduct various games of humans against AI bots to validate the playing strength
of our bot. We are optimistic that our findings can be related to negotiation
...
We thank our supervisor Dr. Iain Philipps (iccp@doc.ic.ac.uk)
for his support and guidance throughout the duration of the
project. For the software engineering aspects of the project Robert
Chatley's course 'Software Engineering Methods' was very instructive, 
improving our process and team collaboration.
...
in post WWI Europe. Each player controls a power which initially
operates from a number of supply centers. The objective of the game is
to control the majority of supply-centers on the map, using army
movements to conquer new bases and extend a power's sphere of
influence. \\
...
their orders and submit them to the server. The submission of orders
happens \textit{simultaneously}; hence players cannot always deduce
which orders led to the final outcome. The diplomacy phase of a game
turn allows powers to signal their actual intention or mislead other
powers about them. In addition to revealing their move orders powers
can also forge alliances and devise strategies
collectively. Ultimately there is no limit to the scope of negotation
messages.

\subsection{Order types / game situations}

\subsubsection{

\todo{Put screenshots from the game, using arrows to indicate standoff
      etc..}




Besides the necessity to create effective
long-term strategies negotiation is integral to the game-experience.
There is a thriving internet-based community around the Game of
Diplomacy and many AI players, so called bots, have been created in
the past. Most of the existing AI players are quite effective
decision-makers in general but few of them support negotiation or
reasoning about the relationship of the powers in the game. Our
project aims to create a framework for Diplomacy, including a GUI
client, a game server and a collection of bots. The highest evolution
of our series of bots should be able to analyze and act upon messages
of human / AI players and issue messages to other players. Finally
experiments are conducted to determine whether there is a reward
associated with negotation capabilities. This required careful study
and implementation of automated negotiation techniques from other
domains in AI / Game Theory. Thus the project gave us ample
...
\subsection{The Game of Diplomacy}

In this section we discuss only the aspects necessary to understand
the algorithms for the AI players. For the definitive reference refer
to \cite{DiploRules00}.

\todo{Re-read rules and finish BG section by tmr}

\subsubsection{Scenario and game objective}

\subsubsection{Game tactics}

\subsubsection{Negotation}
...
\item The over-arching goal and hence the most critical requirement is
  ...
  setting.
\item A collection of other bots which are not capable of commuication
...
\item Finally to test our server and provide a visually pleasing user
  interface we will deliver a DAIDE-conforming GUI client for the Game
...
\item The first iteration -- HoldBot -- is simply an AI player that
  ...
  framework.
\item As a first refinement is RandomBot is created, which now
...
In our project there is a natural division into the negotiation task
and the general game tactics.
