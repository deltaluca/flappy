...
algorithm \cite{Shapiro02} coupled with a strategy-finder is used. It 
supports: goal-directed planning, province valuation. In addition a 
DAIDE-conforming \cite{Daide04} subsystem comprising GUI client and
server was implemented. To our knowledge our server is the first implementation
using the functional language Haskell. Our GUI client is novel in the way
that it can be deployed as a native application as well as for web and smart-phone.
Finally we conducted various games of humans against AI bots to estimate
the playing strength of our bot. The results are included in  a supplementary
paper submitted with the project presentation. On the whole we are optimistic
that our findings can be related to the general research field of \textit{Automated negotiation}
and that our software advances the Diplomacy / DAIDE community on the whole.
